...
$mhsId = $mhs['id'];
$pubId  = (int)($_GET['id'] ?? 0);
if (!$pubId) { header('Location: penelitian'); exit; }

$pub = dbQueryOne(
    "SELECT p.*, m.nama AS nama_mahasiswa, m.nim, m.angkatan, pr.nama AS nama_prodi
     FROM mahasiswa_publikasi p
...
if (!empty($pub['rekan_penulis'])) {
    $coAuthors = array_map('trim', explode(',', $pub['rekan_penulis']));
} else {
    $coAuthors = [$pub['nama_mahasiswa']];
}
$doi       = $pub['doi'] ?? null;

// Build citation string using penulis field (or fallback to student name)
function buildCitation($pub, $coAuthors): string {
    $parts = [];
    // Use the full author list if available
    $authorList = !empty($coAuthors) ? implode(', ', array_map('htmlspecialchars', $coAuthors)) : htmlspecialchars($pub['nama_mahasiswa']);
    if ($pub['dosen_pendamping'] && empty($pub['rekan_penulis'])) {
        $authorList .= ', ' . htmlspecialchars($pub['dosen_pendamping']);
    }
    $parts[] = $authorList . '.';
...
$citationHtml = buildCitation($pub, $coAuthors);
?>
...
<div class="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
    ...
    <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm mb-4">
      ...
      <?php endforeach; ?>
      <?php if ($pub['dosen_pendamping'] && empty($pub['rekan_penulis'])): ?>
      <span class="text-slate-300 dark:text-slate-600">|</span>
      <span class="text-slate-600 dark:text-slate-400 hover:text-[#8c0c4c] dark:hover:text-[#f06ea4] cursor-pointer transition-colors"><?= htmlspecialchars($pub['dosen_pendamping']) ?></span>
      <?php endif; ?>
    </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex gap-8 items-start">
  ...
  <div class="flex-1 min-w-0 space-y-6">
    ...
    <section class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-7 shadow-sm">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-0 divide-y divide-slate-100 dark:divide-slate-700/50 sm:divide-y-0">
        ...
        $infoItems = [
          ...
          ['Program Studi',    $pub['nama_prodi']],
          ['Status',           htmlspecialchars($pub['status_publikasi'])],
          ['Penulis Utama',    htmlspecialchars($coAuthors[0] ?? $pub['nama_mahasiswa'])],
          ['Dosen Pembimbing', $pub['dosen_pendamping'] ? htmlspecialchars($pub['dosen_pendamping']) : null],
    ...
    <section class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-7 shadow-sm">
      ...
      <div class="space-y-4">
        ...
        <!-- BibTeX style -->
        <?php
        $bibtexKey = strtolower(preg_replace('/\W+/','_', substr($coAuthors[0] ?? $pub['nama_mahasiswa'],0,10))) . ($pub['tahun_terbit'] ?: date('Y'));
        $allAuthorsStr = !empty($coAuthors) ? implode(' and ', $coAuthors) : $pub['nama_mahasiswa'];
        if ($pub['dosen_pendamping'] && empty($pub['rekan_penulis'])) $allAuthorsStr .= ' and ' . $pub['dosen_pendamping'];
  ...
  <aside class="hidden lg:block w-72 shrink-0 sticky top-4 space-y-5">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
      ...
      <div class="p-5 space-y-4">
        ...
        foreach ($coAuthors as $idx => $authorName): 
            $isStudent = strcasecmp($authorName, $pub['nama_mahasiswa']) === 0;
            $isDosen   = $pub['dosen_pendamping'] && strcasecmp($authorName, $pub['dosen_pendamping']) === 0;
        ...
        <div class="flex items-start gap-3 <?= $idx > 0 ? 'pt-3 border-t border-slate-100 dark:border-slate-700/50' : '' ?>">
          <div class="w-10 h-10 rounded-full <?= $isStudent ? 'bg-[#8c0c4c] text-white shadow-sm' : ($isDosen ? 'bg-slate-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300') ?> flex items-center justify-center font-bold flex-shrink-0">
            <?= strtoupper(substr($authorName,0,1)) ?>
          </div>
          <div>
            <p class="text-sm font-bold <?= $isStudent ? 'text-slate-800 dark:text-slate-200' : 'text-slate-700 dark:text-slate-300' ?>"><?= htmlspecialchars($authorName) ?></p>
            <div class="flex flex-wrap gap-2 mt-0.5 items-center">
                ...
                <?php if ($isStudent): ?>
                    <span class="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-mono border border-slate-200 dark:border-slate-700">Mahasiswa: <?= htmlspecialchars($pub['nim']) ?></span>
                <?php endif; ?>
                <?php if ($isDosen): ?>
                    <span class="text-[10px] bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-1.5 py-0.5 rounded font-semibold border border-amber-200 dark:border-amber-800/50">Dosen Pembimbing</span>
                <?php endif; ?>
            </div>
          </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm space-y-2">
      ...
      <form action="aksi_publikasi" method="POST" onsubmit="return confirm('Yakin ingin menghapus artikel ini dari portofolio?');">
