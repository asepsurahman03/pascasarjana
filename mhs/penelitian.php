    ...
    if (btnFetch && doiInput) {
        btnFetch.addEventListener('click', async function() {
            ...
            try {
                ...
                // Gabungkan nama jurnal + singkatan untuk pencarian lebih akurat
                ...
                <?php
                // Hitung base URL yang benar untuk lokal & hosting
                // PHP_SELF misal: /Pascasarjana/mhs/penelitian.php
                $baseUrl = rtrim(dirname(dirname($_SERVER['PHP_SELF'] ?? '/mhs/penelitian.php')), '/');
                ?>
                ...
                if (sintaQuery && sintaQuery.length > 2) {
                    ...
                    // Kirim ISSN + nama jurnal (coba keduanya)
                    ...
                    fetch(SINTA_API_URL + '?' + params.toString())
                        ...
                        .catch(err => {
                            console.warn('[SINTA Check] Error:', err);
                            if (noteEl) {
                                noteEl.className = 'mt-2 text-xs font-semibold text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-950/40 p-2.5 rounded-xl border border-amber-200 dark:border-amber-800/60';
