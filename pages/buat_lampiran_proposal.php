<?php
$pageTitle  = 'Buat Lampiran Proposal';
$breadcrumb = [['label'=>'Berkas & Lampiran','url'=>'#'],['label'=>'Riwayat Lampiran Proposal']];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'generate') {
        ...
        $sql = "INSERT INTO riwayat_lampiran (jenis, prodi_id, nama_mhs, nim_mhs, konsentrasi, judul_tesis, tanggal_sidang, ketua_pembimbing, anggota_pembimbing, ketua_penguji, anggota_penguji, created_by) VALUES ('proposal', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        header('Location: buat_lampiran_proposal?step=result&id=' . $id);
        exit;
        ...
        header('Location: buat_lampiran_proposal');
        exit;
...
<div class="max-w-5xl mx-auto pb-10">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-2">📚 Berkas & Lampiran Proposal</h2>
    ...
    <?php if ($step === 'history'): ?>
        <div class="mb-4 flex justify-between items-center">
            <h3 class="text-xl font-bold text-slate-800 dark:text-white">Riwayat Lampiran Proposal</h3>
            <a href="buat_lampiran_proposal?step=form" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-nusa hover:bg-nusa-dark text-white rounded-lg font-semibold transition-all shadow hover:shadow-md text-sm font-bold shadow-lg shadow-blue-500/30">+ Buat Lampiran Baru</a>
        </div>
        
        <?php
        $user = getCurrentUser();
        if (isSuperAdmin()) {
            $riwayat = dbQuery("SELECT r.*, p.nama as nama_prodi FROM riwayat_lampiran r JOIN prodi p ON r.prodi_id = p.id WHERE r.jenis = 'proposal' ORDER BY r.created_at DESC");
        } else {
            $riwayat = dbQuery("SELECT r.*, p.nama as nama_prodi FROM riwayat_lampiran r JOIN prodi p ON r.prodi_id = p.id WHERE r.prodi_id = ? AND r.jenis = 'proposal' ORDER BY r.created_at DESC", [$user['prodi_id']]);
        }
        ?>
...
<tr class="border-b border-slate-200 dark:border-slate-700">
                        <tr class="bg-slate-50 dark:bg-slate-900-bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm text-slate-500 dark:text-slate-400 text-sm border-b border-slate-200 dark:border-slate-700">
                            ...
                            <th class="p-4 font-medium">Judul Proposal</th>
                            <th class="p-4 font-medium">Tanggal Seminar</th>
                            ...
                            <?php foreach ($riwayat as $r): ?>
                                <tr class="hover:bg-slate-50 dark:bg-slate-900-bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm/50 transition">
                                    ...
                                    <td class="py-3 px-4 p-4 text-right space-x-3 whitespace-nowrap">
                                        <a href="buat_lampiran_proposal?step=result&id=<?= $r['id'] ?>" class="text-blue-400 hover:text-blue-300">👁️ Lihat/Cetak</a>
    ...
    <?php if ($step === 'form'): ?>
        ...
        <div class="mb-4">
            ...
            <a href="buat_lampiran_proposal" class="inline-flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg font-semibold transition-all text-sm text-xs inline-flex items-center gap-1">← Kembali ke Riwayat</a>
        </div>
        <form method="POST" class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Proposal & Seminar -->
                <div class="space-y-4">
                    <h3 class="text-lg font-semibold text-green-400 border-b border-slate-200 dark:border-slate-700 pb-2">2. Proposal & Seminar</h3>
                    
                    <div>
                        <label class="block text-sm text-slate-800 dark:text-white mb-1">Judul Proposal <span class="text-red-400">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm text-slate-800 dark:text-white mb-1">Tanggal Seminar <span class="text-red-400">*</span></label>
    ...
    <?php if ($step === 'result'): ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            $lampirans = [
                ...
                ['id'=>4, 'icon'=>'📋', 'title'=>'Lampiran 5', 'desc'=>'Berita Acara Seminar', 'target'=>'Rekapitulasi Nilai Akhir'],
                ['id'=>6, 'icon'=>'📜', 'title'=>'Lampiran 6', 'desc'=>'Lembar Pengesahan', 'target'=>'Persetujuan Tim Penguji'],
                ['id'=>7, 'icon'=>'📝', 'title'=>'Lampiran 7', 'desc'=>'Logbook Bimbingan Proposal Tesis', 'target'=>'Log Konsultasi Proposal'],
                ['id'=>8, 'icon'=>'📝', 'title'=>'Lampiran 8', 'desc'=>'Persetujuan Pembimbing', 'target'=>'Persetujuan Ujian Proposal'],
                ['id'=>9, 'icon'=>'📋', 'title'=>'Lampiran 9', 'desc'=>'Formulir Pendaftaran Seminar', 'target'=>'Kelengkapan Pendaftaran Seminar Proposal'],
                ['id'=>10, 'icon'=>'✅', 'title'=>'Lampiran 10', 'desc'=>'Bukti Kehadiran Seminar', 'target'=>'Daftar Kehadiran Seminar Proposal'],
                ['id'=>11, 'icon'=>'🔏', 'title'=>'Lampiran 11', 'desc'=>'Pernyataan Orisinalitas', 'target'=>'Surat Pernyataan Keaslian Karya Ilmiah'],
            ];
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-5 hover:border-blue-500/50 transition flex flex-col h-full group">
                ...
                <a href="cetak_lampiran_proposal?id=<?= $data['id'] ?>&idx=<?= $l['id'] ?>" target="_blank" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg font-semibold transition-all text-sm border-blue-500/30 text-blue-400 hover:bg-blue-500 hover:text-white justify-center text-sm py-2">
                    👁️ Lihat & Cetak
                </a>
            </div>
            <?php endforeach; ?>
        ...
        <div class="mt-6 flex flex-wrap justify-center gap-4">
            ...
            <a href="cetak_lampiran_proposal?id=<?= $data['id'] ?>&idx=all" target="_blank" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-nusa hover:bg-nusa-dark text-white rounded-lg font-semibold transition-all shadow hover:shadow-md text-sm py-3 px-6 shadow-lg shadow-blue-500/30">
                🖨️ Cetak Semua
            </a>
        </div>
        ...
        function processNextZip(oldText) {
            if (zipQueue.length === 0) {
                ...
                zipInstance.generateAsync({type:"blob"}).then(function(content) {
                    saveAs(content, "Lampiran_Proposal_<?= preg_replace('/[^a-zA-Z0-9]/', '_', $data['nama_mhs']) ?>.zip");
            ...
            currentIframe.src = 'cetak_lampiran_proposal?id=' + id + '&idx=' + idx;
        }
