...
<head>
    ...
    <title>Login - Sistem Pengunduran Diri Mahasiswa | Universitas Nusa Putra</title>
    ...
    <style type="text/tailwindcss">
        ...
        .input-premium {
            @apply w-full py-3.5 rounded-xl border-2 border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800/50 text-slate-800 dark:text-slate-100 transition-all duration-300;
        }
        .input-premium:focus {
            @apply outline-none border-nusa ring-4 ring-nusa/20 bg-white dark:bg-slate-800;
        }
        .btn-premium {
...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 transition-colors duration-300 min-h-screen flex items-center justify-center p-4 sm:p-8 relative overflow-x-hidden" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true' }" :class="{ 'dark': darkMode }">
    ...
    <div class="w-full max-w-6xl glass-panel rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col md:flex-row relative z-10 border border-white/50 dark:border-slate-700/50">
        ...
        <div class="relative w-full md:w-5/12 lg:w-1/2 p-10 md:p-14 flex flex-col justify-center overflow-hidden">
            ...
            <!-- Maroon Overlay -->
            <div class="absolute inset-0 bg-gradient-to-br from-nusa-dark/50 via-nusa/40 to-nusa-light/30 mix-blend-multiply"></div>
            <div class="absolute inset-0 bg-gradient-to-br from-nusa-dark/40 via-nusa/30 to-transparent"></div>
        ...
        <div class="w-full md:w-7/12 lg:w-1/2 p-10 md:p-12 flex flex-col justify-center bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm overflow-y-auto max-h-[85vh] md:max-h-none scroll-smooth" x-data="{ showPass: false, loading: false }" style="scrollbar-width: thin; scrollbar-color: #961d5a transparent;">
            ...
            <div class="max-w-md w-full mx-auto py-4">
                <div class="mb-10 text-center md:text-left">
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm">Masuk ke akun Anda untuk mengakses sistem pengunduran diri.</p>
                </div>
                ...
                <form method="POST" action="<?= APP_URL ?>/?page=auth/login" @submit="loading=true" class="space-y-6">
                    <?= csrfField() ?>

                    <!-- Email / NIM Field -->
                    <div>
                        <label class="block text-slate-700 dark:text-slate-300 text-sm font-bold mb-2 ml-1" for="user-email">Email atau NIM</label>
                        <div class="relative group">
                            ...
                            <input type="text" name="email" id="user-email" required autocomplete="email"
                                value="<?= old('email') ?>"
                                placeholder="Masukkan Email atau NIM Anda"
                                class="input-premium pl-11 pr-4 shadow-sm text-base font-medium">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            ...
                            <input :type="showPass ? 'text' : 'password'" name="password" id="user-pass" required
                                ...
                                class="input-premium pl-11 pr-11 shadow-sm text-base font-medium tracking-wide">
                    ...
                    <div class="flex items-center justify-between ml-1">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            ...
                            <span class="text-slate-600 dark:text-slate-400 text-sm font-semibold group-hover:text-nusa transition-colors">Ingat sesi saya</span>
                        </label>
                    </div>
                    ...
                    <div class="relative flex items-center py-2">
                        ...
                        <span class="flex-shrink-0 mx-4 text-slate-400 dark:text-slate-500 text-xs uppercase font-medium tracking-wider">Atau masuk dengan</span>
                        <div class="flex-grow border-t border-slate-200 dark:border-slate-700"></div>
                    </div>
                    
                    <!-- Google Login Button -->
                    <a href="<?= APP_URL ?>/?page=auth/google" class="w-full flex items-center justify-center gap-3 px-6 py-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-semibold rounded-xl transition-colors shadow-sm focus:ring-4 focus:ring-slate-100 dark:focus:ring-slate-800">
